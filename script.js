...
function start() {
  ...
  while (anotherGame) {
    var anotherGame = confirm('Viltu spila annan leik?');
    if(anotherGame) {
      start();
    }
    else {
...
function play() {
  ...
  console.log('Svarið er = ' + secretNumber);
  let numberOfGuesses = 0;
  let guess = -1;
  while (guess != secretNumber) {
    guess = parseGuess();
    //ef að parseGuess segir að þetta sé ekki tala þá...
    ...
    numberOfGuesses++;
    alert(getResponse(guess,secretNumber));
  }
...
function getResults() {
  ...
  if (games[0] !== 0) {
    return alert('Þú spilaðir ' + numberOfGames + ' leiki\n' + 
